    ...
    public class KompensationBeregner
    {
            ...
            int skatteAar)
        {
            ...
            var bundLettelseBundfradrag
                ...
                skatteydere.Map(
                    skatteyder =>
                        _skattelovRegistry.GetBundLettelseBundfradrag(skatteAar, skatteyder.GetAlder(skatteAar),
                            skatteydere.Size > 1)).ToValueTuple();
        ...
        {
            return indkomster.Map(x => 0.01m * x.AktieIndkomst).ToValueTuple();
        }
        ...
        public ValueTuple<decimal> GetBeskaeftigelsesfradragSkattelettelse(
            ...
            int skatteAar)
        {
            var skattesats = 0.08m + kommunaleSatser.Map(x => x.GetKommuneOgKirkeskattesats()).ToValueTuple();
            ...
            int skatteAar)
        {
            ...
            var personfradragSkaerpelse = skatteydere.Map(skatteyder =>
                _skattelovRegistry.GetPersonfradragSkaerpelse(skatteAar, skatteyder.GetAlder(skatteAar),
                    skatteydere.Size > 1)).ToValueTuple();
            return
                personfradragBeregner.BeregnSkattevaerdierAfPersonfradrag(skatteydere, kommunaleSatser, skatteAar,
                        personfradragSkaerpelse)
                    .Map(x => x.Sum()).ToValueTuple();
        }
            ...
            int skatteAar)
        {
            ...
            var ligningsmaesigeFradrag = indkomster.Map(x => x.LigningsmaessigtFradrag).ToValueTuple();
