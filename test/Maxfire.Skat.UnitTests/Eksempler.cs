...
namespace Maxfire.Skat.UnitTests
{
    ...
    {
        // BUG: Test output (ITestOutputhelper) is only shown when the test fails.
        // See https://github.com/aspnet/Tooling/issues/324 and https://github.com/aspnet/Tooling/issues/541
        private readonly ITestOutputHelper output;

        const int SKATTE_AAR = 2009;
        private readonly ISkattelovRegistry _skattelovRegistry;

        public Eksempler(ITestOutputHelper output)
        {
            this.output = output;
        ...
        public void Eksempel_4_Topskattegrundlag_Gifte()
        {
            ...
            var selvangivneBeloeb = new ValueTuple<ISelvangivneBeloeb>(
                new SelvangivneBeloeb
                {
                    ...
                    LigningsmaessigtFradrag = 1400 // idet besk�ftigelsesfradrag 13.600
                },
                new SelvangivneBeloeb
                {
                    Bruttoloen = 100000,
                    NettoKapitalIndkomst = 23500 - 5000,
                    LigningsmaessigtFradrag = 3750 // idet besk�ftigelsesfradrag 4,25 pct af 100.000
                });
            ...
            indkomster[0].LigningsmaessigtFradrag.ShouldBe(15000); // inkl. beregnet besk�ftigelsesfradrag

            indkomster[1].PersonligIndkomst.ShouldBe(92000);
            indkomster[1].NettoKapitalIndkomst.ShouldBe(18500);
            indkomster[1].LigningsmaessigtFradrag.ShouldBe(8000);  // inkl. beregnet besk�ftigelsesfradrag
        ...
        public void Eksempel_12_NegativSkattepligtigIndkomst_Ugift()
        {
            ...
            // Ingen fremf�rsel af underskud idet hele skattev�rdien kan rummes i bundskatten
            ...
            // Beh�ver ikke reberegne, idet �ndringer i �gtef�lles skattepligtig indkomst, bliver medtaget i denne beregning
            ...
            // Skatter after modregning af underkud, men f�r modregning af personfradrag
        ...
        public void Eksempel_13_HeleUnderskuddetFremfoeres_Ugift()
        {
            ...
            // Skattepligtig indkomst f�r modregning og fremf�rsel
            ...
            // Skatter f�r modregning af underskud og personfradrag er alle nul
            ...
            // Skattepligtig indkomst efter modregning og fremf�rsel
            indkomster[0].SkattepligtigIndkomstSkattegrundlag.ShouldBe(0);
            indkomster[0].SkattepligtigIndkomstUnderskudTilFremfoersel.ShouldBe(59000);

            // Hele �rets underskud i S.I fremf�res
            var underskudTilFremfoersel = modregnResults.Map(x => x.UnderskudTilFremfoersel).ToValueTuple();
        ...
        public void Eksempel_15_FuldModregningAfFremfoertUnderskudOgIntetUnderskudTilFremfoersel_Ugift()
        {
            ...
            // Skattepligtig indkomst f�r modregning og fremf�rsel af underskud
            ...
            // Skattepligtig indkomst efter modregning og fremf�rsel af underskud
        ...
        public void Eksempel_16_ModregningAfFremfoertUnderskudOgUnderskudTilFremfoersel_Ugift()
        {
            ...
            indkomster[0].SkattepligtigIndkomstFremfoertUnderskud.ShouldBe(0);
            skatterAfPersonligIndkomst[0].ShouldBe(IndkomstSkatterAfPersonligIndkomst.Nul);
            // Og dette fremf�res til efterf�lgende skatte�r
            ...
            modregningSkatter[0].Sum().ShouldBe(20832); // svarende til et underskud p� 64515.33m
        ...
        public void Eksempel_17_ModregningAfAaretsOgFremfoertUnderskudOgResterendeUnderskudFremfoeres_Ugift()
        {
            ...
            // Skattepligtig indkomst inden modregning og fremf�rsel
            ...
            // Dette fremf�res til efterf�lgende skatte�r
        ...
        public void Eksempel_18_ModregningFuldtUdViaNedbringelseAfPartnersSkattepligtigeIndkomst()
        {
            ...
            // �1 har ingen egne skatter at modregne underskud i
            skatterAfPersonligIndkomst[0].ShouldBe(IndkomstSkatterAfPersonligIndkomst.Nul);

            // Og hele underskuddet overf�res til �2, hvor det kan indeholdes i �2s skattepligtige indkomst
            var underskudBeregner = new SkattepligtigIndkomstUnderskudBeregner(_skattelovRegistry);
            var modregnResults = underskudBeregner.ModregningAfUnderskud(indkomster, skatterAfPersonligIndkomst, kommunaleSatser, SKATTE_AAR);
            var skatterAfPersonligIndkomstEfterModregningAfUnderskud = modregnResults.Map(x => x.ModregnedeSkatter).ToValueTuple();
        ...
        public void Eksempel_19_ModregningEgenSkatOgPartnersSkattepligtigeIndkomst()
        {
            ...
            // �1 har egne skatter at modregne underskud i
            ...
            // i �gtef�lles indkomst, s� er modregningen af underskud opgjort hos kilden
        ...
        public void Eksempel_20_ModregningPartnersIndkomstOgSkat()
        {
            ...
            // Skattepligtig indkomst f�r modregning af underskud mellem �gtef�ller
            indkomster[0].SkattepligtigIndkomstFremfoertUnderskud.ShouldBe(0);
            ...
            // Der er ingen underskud til fremf�rsel
        ...
        public void Eksempel_21_ModregningEgenSkatOgPartnersIndkomstOgSkat()
        {
            ...
            // �1 har egne skatter at modregne i
        ...
        public void Eksempel_22_ModregningFuldtUdPartnersSkat()
        {
            ...
            skatterAfPersonligIndkomst[1].Bundskat.ShouldBe(1159.20m); // <-- denne skat kan indeholde begge �gtef�llers underskud
            skatterAfPersonligIndkomst[1].Mellemskat.ShouldBe(0);
            ...
            // Output to trace problem

            output.WriteLine("The values should be: {0, 2000, 0}, {0, 1000, 0}");
            output.WriteLine("indkomster[0].SkattepligtigIndkomstSkattegrundlag: {0}", indkomster[0].SkattepligtigIndkomstSkattegrundlag);
            output.WriteLine("indkomster[0].SkattepligtigIndkomstModregninger: {0}", indkomster[0].SkattepligtigIndkomstModregninger);
            output.WriteLine("indkomster[0].SkattepligtigIndkomstUnderskudTilFremfoersel: {0}", indkomster[0].SkattepligtigIndkomstUnderskudTilFremfoersel);
            output.WriteLine("indkomster[1].SkattepligtigIndkomstSkattegrundlag: {0}", indkomster[1].SkattepligtigIndkomstSkattegrundlag);
            output.WriteLine("indkomster[1].SkattepligtigIndkomstModregninger: {0}", indkomster[1].SkattepligtigIndkomstModregninger);
            output.WriteLine("indkomster[1].SkattepligtigIndkomstUnderskudTilFremfoersel: {0}", indkomster[1].SkattepligtigIndkomstUnderskudTilFremfoersel);
...
//#endif
            ...
            //Assert.False(true); // Make the test fail, so that output is shown (BUG in dotnetcli tooling)
            ...
            // S� meget bundskat er der tilbage efter modregning af underskud
        ...
        public void Eksempel_23_DenEnePartnerHarUnderskudFraTidligereAar()
        {
            ...
            // Skattepligtig indkomst f�r modregning af underskud mellem �gtef�ller
            indkomster[0].SkattepligtigIndkomstFremfoertUnderskud.ShouldBe(500000);   // fremf�rt underskud
            indkomster[0].SkattepligtigIndkomst.ShouldBe(173000);                     // �rets underskud
            indkomster[1].SkattepligtigIndkomstFremfoertUnderskud.ShouldBe(0);
            ...
            // Output to trace problem (by outputing the six values below, the compiler optimize problem/bug disappears?????)

            output.WriteLine("The values should be: {0, -173000, 213174.35m}, {0, -67000, 0}");
            output.WriteLine("indkomster[0].SkattepligtigIndkomstSkattegrundlag: {0}", indkomster[0].SkattepligtigIndkomstSkattegrundlag);
            output.WriteLine("indkomster[0].SkattepligtigIndkomstModregninger: {0}", indkomster[0].SkattepligtigIndkomstModregninger);
            output.WriteLine("indkomster[0].SkattepligtigIndkomstUnderskudTilFremfoersel: {0}", indkomster[0].SkattepligtigIndkomstUnderskudTilFremfoersel);
            output.WriteLine("indkomster[1].SkattepligtigIndkomstSkattegrundlag: {0}", indkomster[1].SkattepligtigIndkomstSkattegrundlag);
            ...
            indkomster[1].SkattepligtigIndkomstUnderskudTilFremfoersel.ShouldBe(0);          // 0 in Release
        ...
        public void Eksempel_24_DenEnePartnerHarNegativSkattepligtigIndkomstDenAndenHarEtFremfoertUnderskud()
        {
            ...
            // Skattepligtig indkomst f�r modregning af underskud mellem �gtef�ller
            indkomster[0].SkattepligtigIndkomstFremfoertUnderskud.ShouldBe(0);
            indkomster[0].SkattepligtigIndkomst.ShouldBe(-2000);                      // �rets underskud
            indkomster[1].SkattepligtigIndkomstFremfoertUnderskud.ShouldBe(290000);   // fremf�rt underskud
            indkomster[1].SkattepligtigIndkomst.ShouldBe(250000);

            var skatAfPersonligIndkomstBeregner = new SkatterAfPersonligIndkomstBeregner(_skattelovRegistry);
            var skatterAfPersonligIndkomst = skatAfPersonligIndkomstBeregner.BeregnSkat(indkomster, kommunaleSatser, SKATTE_AAR);

            // Skatter f�r modregning
            ...
            modregningUnderskudSkatter[1].ShouldBe(39021.37m + 978.63m); // egne skatter, derefter �getf�lles skatter
        ...
        public void Eksempel_25_BeggeHarUnderskudFraTidligereAar()
        {
            ...
            // Skattepligtig indkomst f�r modregning af underskud mellem �gtef�ller
            indkomster[0].SkattepligtigIndkomstFremfoertUnderskud.ShouldBe(1090000);  // fremf�rt underskud
            indkomster[0].SkattepligtigIndkomst.ShouldBe(795000);
            indkomster[1].SkattepligtigIndkomstFremfoertUnderskud.ShouldBe(180000);   // fremf�rt underskud
            indkomster[1].SkattepligtigIndkomst.ShouldBe(-70000);                     // �rets underskud
            ...
            modregningUnderskudSkattepligtigIndkomst[1].ShouldBe(68595.23m); // dette bel�b er modregnet i �gtef�lles indkomst
        ...
        public void Eksempel_33_ModregningFuldtUdINettokapitalindkomst_Ugift()
        {
            ...
            // Personlig indkomst og nettokapitalindkomst f�r modregning og fremf�rsel
            ...
            // Personlig indkomst og nettokapitalindkomst efter modregning og fremf�rsel
